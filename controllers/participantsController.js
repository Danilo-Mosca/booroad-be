import { participants } from "../models/dataBooRoad.js";
...
function index(req, res) {
    ...
    if (req.query.search) {
        ...
        data = participants.filter((item) => item.title.toLowerCase().includes(queryString));
    }
    const response = {
        info: {
...
function show(req, res) {
    ...
    const item = participants.find((post) => post.id === id);


    /* Avrei potuto inserire anche il findIndex (che restituisce l'indice dell'oggetto con quello stesso id):
    const itemIndex = participants.findIndex((post)=> post.id === id);
    const item = participants[itemIndex];    // e poi lo assegno alla costante item
    */
